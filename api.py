...
def handle_dialog(req, res):
    ...
    if sessionStorage[user_id]['state'] == 1:
        if req['request']['command'].lower() in [
            ...
            res['response']['text'] = 'Хорошо, вас ждёт 21 вопрос. В каждом вопросе будет 4 варианта ответа, сначала прочитайте все варианты. Выберите одну альтернативу, которая наилучшим образом характеризует ваше состояние в данный момент. Начнем.'
    ...
    if req['request']['command'] in Quest[qid]:
        ...
        if qid == 21:
            ...
            if 0 <= total <= 12:
                ...
                res['response']['text'] = 'По результатам теста могу сделать вывод, что у вас наблюдается легкая стадия депрессии. Для того чтобы избавиться от навязчивого состояния вам нужны: правильный режим сна, здоровая пища и регулярная физическая активность.'
            elif 19 <= total <= 29:
                res['response']['text'] = 'По результатам теста могу сделать вывод, что у вас наблюдается умеренная стадия депрессии. Рекомендую вам озаботиться своим здоровьем, ваше эмоциональное состояние может повлиять на ваше физическое состояние. Зачастую у пациентов, которые находятся в вашем состоянии, наблюдается значительное снижение работоспособности и торможение умственных процессов. Постарайтесть чаще делать физические упражнения, правильно питаться и восстановить свой режим. Также советую избегать конфликтных ситуаций. Если вам не помогают вышепересчисленные меры, обратитесь к психотерапевту'
            else:
                res['response']['text'] = 'По результатам теста могу сделать вывод, что у вас наблюдается тяжелая депрессия. Ваше состояние очень тяжелое, рекомендую обратиться к психотерапевту. 8 (800) 100-49-94. Кризисная линия доверия. Круглосуточно.'
            #res['response']['end_session'] = True
            return 
